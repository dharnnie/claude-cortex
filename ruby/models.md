...
| Bad | Good | Why |
|-----|------|-----|
| `update_attribute` | `update` or `update!` | Skips validations |
| `delete` / `delete_all` | `destroy` / `destroy_all` | Skips callbacks |
| `default_scope` | Named scope | Hard to override |
| `find_by_email(x)` | `find_by(email: x)` | Deprecated syntax |
| `Time.now` | `Time.current` | Timezone aware |
